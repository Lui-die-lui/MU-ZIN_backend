...
public class ArtistStyleService {
    ...
    // 스타일 태그 수정 및 생성
    @Transactional
    public ApiRespDto<?> setMyStyleTags(PrincipalUser principal, ArtistStyleSetRequest req) {
        ArtistProfile profile = artistProfileRepository.findByUser_UserId(principal.getUserId())
                .orElseThrow(()-> new IllegalArgumentException("아티스트 프로필이 없습니다."));
        ...
        if (ids.isEmpty()) {
            return new ApiRespDto<>("success","아티스트 스타일 태그가 비어있습니다.", null);
        }

        // 태그 엔티티 조회
        List<LessonStyleTag> tags = lessonStyleTagRepository.findAllById(ids);
        if (tags.size() != ids.size()){ // 실제 조회댄 태그 개수
            return new ApiRespDto<>("failed", "존재하지 않는 스타일 태그가 포함되어 있습니다.", null);
        }
        ...
        return new ApiRespDto<>("success", "아티스트 스타일 태그가 저장되었습니다.", null);
